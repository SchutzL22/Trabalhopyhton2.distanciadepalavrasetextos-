# Trabalho 2 - Distância de palavras e textos
...
def obter_frequencia_letras(palavra):
    ...
    for letra in palavra:
        if letra in frequencialetra:
            frequencialetra[letra] += 1
        else:
            frequencialetra[letra] = 1

    return frequencialetra
...
def palavras_mais_frequentes(dicionario1, dicionario2):
    ...
    """
    freq_total = dicionario1.copy()

    for palavra, freq in dicionario2.items():
        freq_total[palavra] = freq_total.get(palavra, 0) + freq
    ...
    for palavra, freq in freq_total.items():
...
def obter_tf(arquivo):
    ...
    * Use a função obter_frequencia definida antes.
    ...
    palavras = texto_para_lista(texto)
    freq_dict = obter_frequencia(palavras)
    ...
    for palavra, freq in freq_dict.items():
        tf_dict[palavra] = freq / total_palavras
...
def obter_idf(arquivos):
    ...
    * IDF é calculado como IDF(i) = log_10(número total de documentos / número de documentos 
    ...
    for doc in arquivos:
        texto = carregar_arquivo(doc)
        palavras = texto_para_lista(texto)
        palavras_unicas_doc = set(palavras)
        
        for palavra in palavras_unicas_doc:
            doc_contem_palavra[palavra] = doc_contem_palavra.get(palavra, 0) + 1
            
    idf_dict = {}
    
    for palavra, count in doc_contem_palavra.items():
        idf_dict[palavra] = math.log10(num_documentos / count)
...
def obter_tfidf(arquivo_tf, arquivos_idf):
    ...
    * TF-IDF(i) = TF(i) * IDF(i)
    """
    tf_dict = obter_tf(arquivo_tf)
    idf_dict = obter_idf(arquivos_idf)
    ...
    for palavra, tf_val in tf_dict.items(): 
        ...
        tfidf_list.append((palavra, tfidf))
...
if __name__ == "__main__":
    ...
    diretorio_teste = os.path.join(os.path.dirname(os.path.abspath(__file__)), "testes", "testes_estudantes", "")
    ...
    tf_arquivo = os.path.join(diretorio_teste, 'ola_mundo.txt')
    idf_arquivos = [os.path.join(diretorio_teste, 'ola_mundo.txt'), os.path.join(diretorio_teste, 'ola_amigos.txt')]
